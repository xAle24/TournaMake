...
class PlayerActActivity : ComponentActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            matchListViewModel.matchListLiveData.observe(this, matchObserver)
            fetchAndUpdateMatch(loggedEmail.value.loggedProfileEmail, matchListViewModel)
            MatchListScreen(
                state = state.value,
                matchListLiveData = matchListViewModel.matchListLiveData,
    ...
    private fun fetchAndUpdateMatch(email: String, matchListViewModel: MatchListViewModel) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val myMatch = appDatabase?.matchDao()?.getMyMatch(email)
                ...
                // observer
                matchListViewModel.changeMatchList(myMatch)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
